...
| Key | Action |
|---|---|
| Return | Open the details + action menu for that reminder |
...
| ⇥ Tab | Open the details + action menu for that reminder (same destination as Return) |
...
more info — now Return and Tab both drill into the same screen. Since one
Alfred item can only carry a single `autocomplete` value, the only way to
make Return "enter" a screen rather than fire an action immediately is to
make the base item `valid: false` with `autocomplete` pointing at that
screen — which is why both keys now land in the same place rather than
diverging. Per Alfred's own docs, `autocomplete` is a Tab-triggered
...
The details + action menu, in order:

1. Read-only lines: **Title**, **List**, **Due**, **Priority**, **Flagged**,
   **Tags**, **Notes** — Return on any of these opens the reminder in
   Reminders.app, same as the old standalone "View details" screen.
2. **Mark as complete**
3. **Reschedule…**
4. **Change title…**
5. **Move to another list…**
6. **Open in Reminders.app**

Reschedule, change-title, and move all need a follow-up value or more
screen space than a modifier+Return can give (a modifier is a one-shot
fire-and-forget action, not an interactive prompt or a second screen),
which is why they live in the menu rather than on a modifier key. Tab
into the menu, then Tab again on "Reschedule…" / "Change title…" /
"Move to another list…" drops you into a text-entry prompt or picker —
these are `valid: false` items, so only Tab reliably applies their
`autocomplete`. To back out of any of these without finishing, just
...
skip straight to executing instead. Opening a reminder (from a detail
line, or "Open in Reminders.app") is never confirmed — it doesn't change
anything.
...
routing anywhere. `list_reminders.py` handles browse, the details+action
menu (reached via Tab or Return), the edit/reschedule/move text-entry
prompts and picker, *and* the confirm-step summary all in one script,
branching on a prefix in the query string itself (`menu:<id>`,
`edit:<id>:<text>`, `due:<id>:<text>`, `movelist:<id>:<text>`,
...
`remctl` to mutate anything — the read-only detail lines in the menu
share the `open` action, so selecting one just opens Reminders.app rather
...
python3 list_reminders.py "menu:23880"                     # details + action menu
